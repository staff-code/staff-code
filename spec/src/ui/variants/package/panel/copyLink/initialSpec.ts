...
describe("computeInitialParams", (): void => {
    ...
    it("returns an empty string when all UI components are at default settings, and the only UI component which exists is the display", (): void => {
    ...
    it("returns an empty string when all UI components are at default settings", (): void => {
        mockDom()
    ...
    it("returns params for any UI component which is not at its default settings", (): void => {
        mockDom()
        ...
        const expected = "?size=1.13&line=2.09&reference-open=true&codes=ston-Gcl-q-nt" as string
        expect(actual).toBe(expected)
    })
