...
const removeVowelsAndVowelSoundCharSequencesBesidesFirstChar = (string: Name<Unicode> & Word): Name<Unicode> & Word => {
    const replaced = string
        .slice(1)
        .replace(/igh/g, BLANK)         // 100x right, 3x tight, 2x eight, 6x high, 3x higher, 2x highest
        .replace(/[aeiou]/g, BLANK)

    return `${string[0]}${replaced}` as Name<Unicode> & Word
}
...
const computeGlyphNameWordAbbreviation = (word: Name<Unicode> & Word): Abbreviation<Unicode> & Word => {
    ...
    if (lowercasedWord.length === 2) return lowercasedWord as Word as Abbreviation<Unicode> & Word
    ...
    if (!isUndefined(maybeException)) return maybeException[1] as Abbreviation<Unicode> & Word
